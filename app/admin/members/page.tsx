...
export default function AdminMembersPage() {
  ...
  // Announcement email
  const [announcing, setAnnouncing] = useState<'test' | 'all' | null>(null)
  ...
  const handleTestAnnounce = async () => {
    ...
    try {
      ...
      const data = await res.json()
      setAnnounceResult(res.ok ? `✓ Test sent to ${data.sent_to}` : `Error: ${data.error}`)
    } catch {
  ...
  const handleAnnounceAll = async () => {
    if (!confirm(`Send the announcement email to all ${members.length} members? This cannot be undone.`)) return
    setAnnouncing('all')
    setAnnounceResult('')
    try {
      const res = await fetch('/api/members/announce', {
      ...
      const data = await res.json()
      setAnnounceResult(res.ok ? `✓ Sent to ${data.sent} members (${data.failed} failed)` : `Error: ${data.error}`)
    } catch {
      setAnnounceResult('Network error.')
    } finally {
      setAnnouncing(null)
  ...
  return (
    <div className="min-h-screen bg-gray-900">
      ...
      <main className="max-w-7xl mx-auto px-6 py-10 space-y-6">
        {/* Announcement panel */}
        <div className="card border-brand-red/30 bg-gradient-to-br from-brand-red/5 to-transparent">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h2 className="text-base font-semibold text-white flex items-center gap-2">
                <span className="text-lg">📣</span> Chapter Announcement Email
              </h2>
              <p className="text-sm text-gray-400 mt-1">
                Send the platform showcase email to members. Always test first, then send to everyone.
              </p>
              {announceResult && (
                <p className={`text-sm mt-2 ${announceResult.includes('✓') ? 'text-green-400' : 'text-red-400'}`}>
                  {announceResult}
                </p>
              )}
            </div>
            <div className="flex gap-3 flex-shrink-0">
              <button
                ...
                disabled={announcing !== null}
                className="btn-ghost text-sm px-4 py-2 disabled:opacity-50"
              >
                {announcing === 'test' ? 'Sending…' : 'Send Test to Me'}
              </button>
              <button
              ...
              >
                {announcing === 'all' ? 'Sending…' : `Send to All ${members.length} Members`}
              </button>
            </div>
          </div>
        </div>
